...
// almost same to mptcp_sched.c except for get_available_subflow function
...
static unsigned int ecf_optim __read_mostly = 0;
module_param(ecf_optim, int, 0644);
MODULE_PARM_DESC(ecf_optim, "optimization for ECF");
 ...
 */
static struct sock *default_get_available_subflow(struct sock *meta_sk,
					  struct sk_buff *skb,
					  bool zero_wnd_test)
{
	...
	// ylim: some procedures are removed since those are done in get_available_subflow
		...
		if( !(tp->mptcp->rcv_low_prio || tp->mptcp->low_prio) ) {
			if( mptcp_is_available(fastest_sk, skb, zero_wnd_test) ) {
				if( !mptcp_dont_reinject_skb(tp, skb) ) {
					...
					u32 cwnd_f = ecf_optim ? max( tcp_sk(fastest_sk)->snd_cwnd, tcp_sk(fastest_sk)->snd_cwnd_before_idle_restart ) : tcp_sk(fastest_sk)->snd_cwnd;
...
/*			u32 mss = ( (u32) tcp_current_mss(bestsk) );
			...
			u32 srtt_f = tcp_sk(fastest_sk)->srtt >> 3;
			u32 rttvar_f = tcp_sk(fastest_sk)->rttvar >> 1;

			bestsk = fastest_sk;

			mptcp_debug("%s: %pI4:%d -> %pI4:%d, fastest is used(def) %u %u %u %u %u\n",
			    __func__ , 
			    &((struct inet_sock *) bestsk )->inet_saddr,
			    ntohs(((struct inet_sock *) bestsk )->inet_sport),
			    &((struct inet_sock *) bestsk )->inet_daddr,
...
*/
			...
			if(bestsk != NULL && fastest_sk != NULL) {
				...
				u32 cwnd_s = ecf_optim ? max( tcp_sk(bestsk)->snd_cwnd, tcp_sk(bestsk)->snd_cwnd_before_idle_restart ) : tcp_sk(bestsk)->snd_cwnd;
...
static struct sk_buff *mptcp_rcv_buf_optimization(struct sock *sk, int penal)
{
	...
	struct ecf_sched_priv *esp = ecf_sched_get_priv(tp);
	...
	if (tcp_time_stamp - esp->last_rbuf_opti < usecs_to_jiffies(tp->srtt_us >> 3))
		goto retrans;
	...
	mptcp_for_each_tp(tp->mpcb, tp_it) {
		if (tp_it != tp &&
			...
			if (tp->srtt_us < tp_it->srtt_us && inet_csk((struct sock *)tp_it)->icsk_ca_state == TCP_CA_Open) {
				...
				esp->last_rbuf_opti = tcp_time_stamp;
			}
			break;
		}
	}
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("ECF (Earliest Completion First Scheduler) for MPTCP, some codes from Default Scheduler");
MODULE_VERSION("0.89");
